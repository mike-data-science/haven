...
export default function DiscoverClient({
  ...
  return (
    <div className="font-sans text-slate-200 min-h-full pb-8 max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 mt-6">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold font-serif mb-1 tracking-tight text-white">Discover Properties</h1>
          <p className="text-slate-400 text-sm">Explore our curated selection of premium real estate.</p>
        </div>
      </div>

      {/* Top Filter Bar */}
      <div className="flex items-center gap-8 mb-6 border-b border-slate-800 w-full overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      ...
      <div className="flex flex-col xl:flex-row gap-6 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center justify-between mb-6 gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-slate-400">Map View</span>
              <button
                onClick={() => setMapView(!mapView)}
                className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors shadow-inner border border-white/5 ${mapView ? "bg-[var(--theme-accent)]" : "bg-slate-800"}`}
              >
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            <div className="flex-1 min-w-[200px] relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2.5 rounded-full bg-[#1A1A18] border border-slate-800 focus:outline-none focus:border-[var(--theme-accent)] text-white text-xs font-medium placeholder-slate-500 transition-colors"
              />
            </div>
            <button className="px-4 py-2.5 rounded-full bg-[#1A1A18] border border-slate-800 text-xs font-semibold text-slate-300 hover:bg-slate-800 flex items-center gap-1.5 transition-colors">
            ...
            <button className="px-4 py-2.5 rounded-full bg-[#1A1A18] border border-slate-800 text-xs font-semibold text-slate-300 hover:bg-slate-800 flex items-center gap-1.5 transition-colors">
              Property Type <ChevronDown className="h-3 w-3 text-slate-500" />
            </button>
          </div>
            ...
            {["Recommended", "Popular", "Nearest"].map((tab) => (
              <button
                ...
                className={`text-xs font-bold transition-colors whitespace-nowrap px-3 py-1.5 rounded-full ${
                  subTab === tab ? "bg-[var(--theme-accent)]/10 text-[var(--theme-accent)]" : "text-slate-500 hover:text-slate-300"
                }`}
              >
                {tab}
              </button>
          ...
          {paginatedListings.length === 0 ? (
            <div className="text-center py-12 bg-[#1A1A18] rounded-3xl border border-slate-800">
          ...
          {totalPages > 1 && (
            <div className="mt-8 flex justify-center items-center gap-2">
              <button
                ...
                className="w-8 h-8 flex items-center justify-center rounded-full bg-[#1A1A18] border border-slate-800 text-slate-400 hover:text-white hover:bg-slate-800 disabled:opacity-50 disabled:pointer-events-none transition-colors"
              >
                    ...
                    <button
                      ...
                      className={`w-8 h-8 flex items-center justify-center rounded-full text-sm font-semibold transition-colors ${
                        currentPage === page 
                          ...
                          : "text-slate-400 hover:text-white hover:bg-[#1A1A18]"
                      }`}
                    >
                      {page}
                    </button>
        ...
        <div className="w-full xl:w-[450px] flex flex-col gap-5 shrink-0 min-w-0">
          {mapView && (
            <div className="bg-[#1A1A18] p-4 rounded-3xl border border-slate-800 shadow-xl flex-1 min-h-[500px] xl:sticky xl:top-6">
              ...
              <div className="h-[calc(100%-2rem)] rounded-2xl overflow-hidden min-h-[400px] border border-slate-800">
                <UniversalMap
                  mode="listings"
